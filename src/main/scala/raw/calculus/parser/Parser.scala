 ...
 *
 *  TODO: [MSB] Fix handling of badly-formed expressions in constant
 *
 */
...
case class ConversionMismatch(from: MonoidType, to: MonoidType) extends RawException("conversion mismatch from " + from + " to " + to)
...
class Parser(val rootScope: RootScope) extends StandardTokenParsers {
   ...
   *
   */
  def cast(t: MonoidType, e: TypedExpression) = e.monoidType match {
    case BoolType => t match {
      case BoolType => e
      case _        => throw ConversionMismatch(e.monoidType, t)
    }
    case IntType => t match {
      case FloatType  => IntToFloat(e)
      case IntType    => e
      case BoolType   => throw ConversionMismatch(e.monoidType, t)
      case StringType => IntToString(e)
    }
    case FloatType => t match {
      case FloatType  => e
      case IntType    => FloatToInt(e)
      case BoolType   => throw ConversionMismatch(e.monoidType, t)
      case StringType => FloatToString(e)
    }
    case StringType => t match {
      case FloatType  => StringToFloat(e)
      case IntType    => StringToInt(e)
      case BoolType   => StringToBool(e)
      case StringType => e
