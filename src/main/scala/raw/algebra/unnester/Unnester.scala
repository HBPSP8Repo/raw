...
class Unnester(cat: Catalog) {
  ...
  private def convertType(t: calculus.MonoidType): algebra.ExpressionType = t match {
    ...
    case calculus.RecordType(atts) => algebra.RecordType(atts.map(att => algebra.Attribute(att.name, convertType(att.monoidType))))
    case calculus.SetType(t)       => algebra.SetType(convertType(t))
    case calculus.BagType(t)       => algebra.BagType(convertType(t))
    case calculus.ListType(t)      => algebra.ListType(convertType(t))
  }

  private def convertVariable(v: calculus.canonical.Variable, p: Pattern) = {
    val idx = getPatternVariables(p).indexOf(v)
    algebra.Argument(convertType(v.monoidType), idx)
  }
  ...
  private def convertExpression(e: calculus.canonical.TypedExpression, p: Pattern): algebra.Expression = {
    e match {
      ...
      case calculus.canonical.RecordConstruction(t, atts)    => algebra.RecordConstruction(convertType(t), atts.map(att => algebra.AttributeConstruction(att.name, convertExpression(att.e, p))))
      case calculus.canonical.IfThenElse(t, e1, e2, e3)      => algebra.IfThenElse(convertType(t), convertExpression(e1, p), convertExpression(e2, p), convertExpression(e3, p))
      case calculus.canonical.BinaryOperation(t, op, e1, e2) => algebra.BinaryOperation(convertType(t), op, convertExpression(e1, p), convertExpression(e2, p))
      case calculus.canonical.MergeMonoid(t, m, e1, e2)      => algebra.MergeMonoid(convertType(t), convertMonoid(m), convertExpression(e1, p), convertExpression(e2, p))
