...
class SelectWhereTest extends AbstractSparkPublicationsTest {
  ...
  def convertAuthors(iterator: java.util.Iterator[Author]): String = {
    JavaConversions.asScalaIterator[Author](iterator)
  ...
  def convertPublications(iterator: java.util.Iterator[Publication]): String = {
    JavaConversions.asScalaIterator[Publication](iterator)
      .map(p => s"${p.title}; ${listToString(p.authors)}; ${listToString(p.affiliations)}; ${listToString(p.controlledterms)}")
      .toList
      .sorted
      .mkString("\n")
  }
  ...
  test("SelectWhere0") {
    ...
    val actual = convertAuthors(authors.iterator())
...
//    val actual = convertPublications(pubs.iterator())
//
