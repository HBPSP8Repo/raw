...
@rawQueryAnnotation
class Select0Query(val patients: RDD[Patient]) extends RawQuery {
  val oql = """
    count(patients)
  """
}
...
@rawQueryAnnotation
class Select1Query(val patients: RDD[Patient]) extends RawQuery {
  val oql = """
...
@rawQueryAnnotation
class Select2Query(val patients: RDD[Patient]) extends RawQuery {
  val oql = """
...
class SelectTest extends AbstractSparkPatientsTest {
  ...
  test("Select0") {
    val result = new Select0Query(patientsRDD).computeResult
  ...
  test("Select1") {
    val result = new Select1Query(patientsRDD).computeResult
    val actual = convertToString(result)
  ...
  test("Select2") {
    val result = new Select2Query(patientsRDD).computeResult
    val actual = convertToString(result)
    val expected = convertExpected("""
