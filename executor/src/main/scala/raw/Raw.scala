...
abstract class RawQuery extends Iterable[Any] with StrictLogging {
  override def iterator: RawQueryIterator

  // autogenerated
  var compilationTime: Long = -1
  var executionTime: Long = -1

  /* The user running this query. The class code-generated for this query overrides this field.
     This is used to lookup the datasources in the MD catalog
   */
  val user: String

  def isPrimitive(value: Any): Boolean = {
    !value.isInstanceOf[AnyRef] || ClassUtils.isPrimitiveOrWrapper(value.getClass)
  }

  // TODO: This is a temporary fix that preserves the old behaviour until all the client code is updated to use iterators
  def computeResult: Any = {
    ...
    try {
      ...
      val res = if (list.size == 1 && isPrimitive(list.head)) {
        list.head
      } else {
        list
      }
      executionTime = start.elapsed(TimeUnit.MILLISECONDS)
      logger.info(s"Execution time: $executionTime ms")
      res
    } finally {
      iter.close()
    }
...
// TODO: Read on the correct way to implement an iterator. There are many corner-cases.
abstract class RawQueryIterator(query: RawQuery) extends Iterator[Any] with StrictLogging {
  private[this] val openIters = new ArrayBuffer[AbstractClosableIterator[_]]()
  ...
  protected[this] def openSource[T: ClassTag : TypeTag : Manifest](schemaName: String): Iterable[T] = {
    logger.info("Opening schema name: " + schemaName)
    val dataSource = MDCatalog.lookup(query.user, schemaName).get
    val scanner = RawScanner(dataSource)
    new RawIterable(scanner, openIters).view
  }
  ...
  protected def executeQuery(): Any // Autogenerated

  override def hasNext: Boolean = !closed && queryIterator.hasNext
  ...
  private[this] def open(): Iterator[Any] = {
    val res = executeQuery
    try {
      val iterable = res.asInstanceOf[Iterable[_]]
      iterable.toIterator
    } catch {
      case ex: ClassCastException => Iterator.single(res)
    }
  }
...
class RawImpl(val c: scala.reflect.macros.whitebox.Context) extends StrictLogging {
  ...
  /** Collect all record types in the tree.
    ...
    */
  def collectRecordTypes(tree: Calculus.Exp, analyzer: SemanticAnalyzer): Seq[RecordType] = {
    import org.kiama.rewriting.Rewriter._

    val recordTypes = scala.collection.mutable.ListBuffer[RecordType]()

    def extractClasses(e: raw.Type): Unit = {
      e match {
        case r: RecordType => {
          recordTypes += r
          r.recAtts.atts.foreach(att => extractClasses(att.tipe))
        }
        case CollectionType(_, innerType: raw.Type) => extractClasses(innerType)
        case t =>
      }
    }

    val queryRecordTypes = everywhere(query[Calculus.Exp] {
      case e: Calculus.Exp => extractClasses(analyzer.tipe(e))
    })
  ...
  def buildCaseClasses(tree: Calculus.Exp, world: World, analyzer: SemanticAnalyzer): Set[Tree] = {
    ...
    val recordTypes: Seq[RecordType] = collectRecordTypes(tree, analyzer)
    logger.info(s"Record types: $recordTypes")
    ...
    for ((name, r) <- toBuild) {
      ...
      val args = r.recAtts.atts.map(att => {
        val scalaType = buildScalaType(att.tipe, world, analyzer)
        s"${att.idn}: ${scalaType}"
      }).mkString(", ")
      val caseClassSrc = s"case class $name($args)"
      code += caseClassSrc
    }
  ...
  /** Build code-generated query plan from logical algebra tree.
    */
  def buildCode(treeExp: Calculus.Exp, user: String, world: World, analyzer: PhysicalAnalyzer): Tree = {
    import Calculus._
    ...
    def buildScan(e: IdnExp) = e match {
      ...
      case IdnExp(IdnUse(idn)) if !analyzer.spark(e) =>
        ...
        //        q"""openScanner($ident)"""
        //        q"""openSource("${idn}")"""
        // TODO: Obtain type of idn.
        val schemaInfo = MDCatalog.lookup(user, idn).get
        val outType: raw.Type = analyzer.tipe(e)
        val innerStr = outType match {
          case CollectionType(_, innerType) => buildScalaType(innerType, world, analyzer)
        }
        //        c.parse( s"""openSource[${schemaInfo.innerType}]("$idn")""")
        c.parse( s"""openSource[${innerStr}]("$idn")""")
  ...
  def extractQueryAndAccessPath(makro: Tree): (String, String, QueryLanguage, List[AccessPath]) = {
    /**
      ...
      case ClassDef(_, className: TypeName, _, Template(_, _, body: List[Tree])) =>
        ...
        body.foreach(v => {
          ...
          v match {
            ...
            case ValDef(_, TermName("user "), _, Literal(Constant(user: String))) =>
              logger.info("Found username : " + user)
              queryUser = user
        ...
        (query.get, queryUser, queryLanguage.get, accessPaths.toList)
    }
  ...
  def generateCode(makro: Tree, query: String, user: String, queryLanguage: QueryLanguage, accessPaths: List[AccessPath]): c.Expr[Any] = {
    //    val sources: Map[String, raw.Type] = accessPaths.map(ap => (ap.name, ap.rawType)).toMap
    val dataSources: scala.Seq[DataSource] = MDCatalog.listUserSchemas(user)
    val sources: Map[String, raw.Type] = dataSources.map(ds => (ds.name, ds.tipe)).toMap
    //    val sources: Map[String, raw.Type] = accessPaths.map(ap => (ap.name, MDCatalog.lookup(ap.name).get.tipe)).toMap
    ...
    parseResult match {
      case Right(tree) =>
        ...
        // TODO: Retrieve from MD catalog
        val isSpark: Map[String, Boolean] = dataSources.map(ds => (ds.name, false)).toMap
        logger.info(s"Is Spark: $isSpark")
        ...
        logger.info(s"Classes map: $classesMap")
        val generatedTree: Tree = buildCode(treeExp, user, world, physicalAnalyzer)
  ...
  object $moduleName {
     ..$caseClasses
        ...
        val iteratorClass = TypeName(s"${moduleName}Iterator")
        val clazz =
          q"""
  class $className(..$methodDefParameters) extends RawQuery {
    import raw.RawInt
    import ${moduleName}._
    ..$body
    override def iterator = new $iteratorClass(this)

    class $iteratorClass(query:RawQuery) extends raw.RawQueryIterator(query) {
      val queryLogger = org.slf4j.LoggerFactory.getLogger("raw.queries")
  ...
  """
        //        def executeQuery(..$methodDefParameters) = {
        //    $generatedTree
        //    }
        //
        //    def computeResult = {
      ...
      case Left(err) => {
        logger.info(s"Error parsing query: $err")
        bail(err)
      }
    }
  }
  ...
  def query_impl(annottees: c.Expr[Any]*): c.Expr[Any] = {
    ...
    try {
      ...
      val (query: String, user: String, language: QueryLanguage, accessPaths: List[AccessPath]) = extractQueryAndAccessPath(makro)
      logger.info("Access paths: {}", accessPaths.map(ap => ap.name + ", isSpark: " + ap.isSpark).mkString("; "))
      generateCode(makro, query, user, language, accessPaths)
    } catch {
