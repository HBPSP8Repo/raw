...
object RawUtils extends StrictLogging {
  def cleanOrCreateDirectory(p: Path) = {
    if (Files.isDirectory(p)) {
      logger.debug(s"Directory $p already exists. Deleting contents.")
  ...
  def createDirectory(p: Path) = {
    if (Files.isDirectory(p)) {
      val dirContents = util.Arrays.toString(Files.list(p).toArray)
      logger.debug(s"Directory already exists. Contents: $dirContents")
    } else {
      logger.debug(s"Creating directory tree: $p")
  ...
  def extractResource(resourceName: String): Path = {
    logger.info(s"Extracting resource to temporary directory: $resourceName")
