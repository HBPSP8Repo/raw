...
class LocalStorageManager(val storagePath: Path = StorageManager.defaultDataDir)
  ...
  /** Register schemaName belonging to user rawUSer. The staging directory should contain three files:
    * - the data file
    * - schema.xml
    * - properties.json
    *
    * The last two files must have been generated by the interrer previously.
    *
    * This method copies the schema files into it's own storage directory. Alternative implementations could store
    * this information in a more compact fashion (ie., in a DB)
    */
  override def registerSchema(schemaName: String, stagingDirectory: Path, rawUser: String) = {
    logger.info(s"Registering schema: $schemaName, stageDir: $stagingDirectory, user: $rawUser")
    val userDataDir = getUserStorageDir(rawUser)
    ...
    val dataSource = loadSchemaFromStorage(rawUser, schemaName)
    MDCatalog.register(rawUser, schemaName, dataSource)
  }
