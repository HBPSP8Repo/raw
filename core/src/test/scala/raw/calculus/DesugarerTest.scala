...
class DesugarerTest extends FunTest {
  ...
  test("desugar ExpBlock") {
    compare(
      process("""for (d <- Departments) yield set { name := d.name; (deptName := name) }""", TestWorlds.departments),
  ...
  test("desugar PatternGen") {
    compare(
      process("""for ((a, b, c, d) <- set_of_tuples) yield set (a, d)""", TestWorlds.set_of_tuples),
