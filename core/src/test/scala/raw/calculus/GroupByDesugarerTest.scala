...
class GroupByDesugarerTest extends CalculusTest {
  ...
  test("select s.age/10, * from students s group by s.age/10") {
    check(
      ...
      """select (_1: s$0.age / 10, _2: select * from $1 <- students where s$0.age / 10 = $1.age / 10) from s$0 <- students""",
  ...
  test("select s.age/10, partition from students s group by s.age/10") {
      ...
      """select (_1: s$0.age / 10, partition: select $1 from $1 <- students where s$0.age / 10 = $1.age / 10) from s$0 <- students""",
      TestWorlds.professors_students)
  }
  ...
  test("select s.age/10, partition from students s, professors group by s.age/10") {
    check(
      ...
      """select (_1: s$0.age / 10, partition: select (s: $1, _2: $2) from $1 <- students, $2 <- professors where s$0.age / 10 = $1.age / 10) from s$0 <- students, $3 <- professors""",
      TestWorlds.professors_students)
  }
  ...
  test("select s.age/10, partition from students s, professors p group by s.age/10") {
    check(
      ...
      """select (_1: s$0.age / 10, partition: select (s: $1, p: $2) from $1 <- students, $2 <- professors where s$0.age / 10 = $1.age / 10) from s$0 <- students, p$0 <- professors""",
      TestWorlds.professors_students)
  }
  ...
  test("select s.age/10 as decade, partition as students from students s group by s.age/10") {
    check(
      ...
      """select (decade: s$0.age / 10, students: select $1 from $1 <- students where s$0.age / 10 = $1.age / 10) from s$0 <- students""",
      TestWorlds.professors_students)
  }
  ...
  test("select s.age/10 as decade, (select s.name from partition s) as names from students s group by s.age/10") {
    check(
      ...
      """select (decade: s$0.age / 10, names: select s$1.name from s$1 <- select $2 from $2 <- students where s$0.age / 10 = $2.age / 10) from s$0 <- students""",
      TestWorlds.professors_students)
  }
    ...
    check(
      ...
      """select (age: s$0.age,
                 names: select (name: s$1.name,
                                partition: select $2 from $2 <- select $3 from $3 <- students where s$0.age = $3.age where s$1.name = $2.name)
                        from s$1 <- select $3 from $3 <- students where s$0.age = $3.age)
         from s$0 <- students""",
      TestWorlds.professors_students)
  }
    ...
    check(
      ...
      """select (age: s$0.age,
                 names: select (name: p$0.name,
                                partition: select $1 from $1 <- select $2 from $2 <- students where s$0.age = $2.age where $1.age > 30 and p$0.name = $1.name)
                        from p$0 <- select $2 from $2 <- students where s$0.age = $2.age where p$0.age > 30)
         from s$0 <- students""",
      TestWorlds.professors_students)
  }
  ...
  test("select A.title, count(partition) as n from authors A group by A.title") {
    check(
      ...
      """select (title: A$0.title, n: count(select $1 from $1 <- authors where A$0.title = $1.title)) from A$0 <- authors""",
  ...
  test("select A.title, partition as people from authors A group by A.title") {
    check(
      ...
      """select (title: A$0.title, people: select $1 from $1 <- authors where A$0.title = $1.title) from A$0 <- authors""",
      TestWorlds.publications
    )
  }
  ...
  test("select A.title, (select A.year from A in partition) as years from authors A group by A.title") {
    check(
      ...
      """ select (title: A$0.title, years: select A$1.year from A$1 <- select $2 from $2 <- authors where A$0.title = $2.title) from A$0 <- authors""",
      TestWorlds.publications)
  }
  ...
  test("select *, partition from students s group by s.age") {
    check(
      ...
      """select (_1: select * from $0 <- students where s$0.age = $0.age, partition: select $2 from $2 <- students where s$0.age = $2.age) from s$0 <- students""",
      TestWorlds.professors_students)
  }
  ...
  test("select partition, * from students s group by s.age") {
    check(
      """select partition, * from students s group by s.age""",
      """select (partition: select $0 from $0 <- students where s$0.age = $0.age, _2: select * from $2 <- students where s$0.age = $2.age) from s$0 <- students""",
      TestWorlds.professors_students)
  }
  ...
  test("select *, * from students s group by s.age") {
    check(
      ...
      """select (_1: select * from $0 <- students where s$0.age = $0.age, _2: select * from $0 <- students where s$0.age = $0.age) from s$0 <- students""",
      TestWorlds.professors_students,
  ...
  test("select count(*), *, partition from students s group by s.age") {
    check(
      ...
      """select (_1: count(select * from $0 <- students where s$0.age = $0.age), _2: select * from $0 <- students where s$0.age = $0.age, partition: select $2 from $2 <- students where s$0.age = $2.age) from s$0 <- students""",
      TestWorlds.professors_students)
  }
  ...
  test("select *, partition from students s, professors group by s.age") {
    check(
      ...
      """select (_1: select * from $0 <- students, $1 <- professors where s$0.age = $0.age, partition: select (s: $3, _2: $4) from $3 <- students, $4 <- professors where s$0.age = $3.age) from s$0 <- students, $2 <- professors""",
      TestWorlds.professors_students)
  }
  ...
  test("select partition, * from students s, professors group by s.age") {
    check(
      """select partition, * from students s, professors group by s.age""",
      """select (partition: select (s: $0, _2: $1) from $0 <- students, $1 <- professors where s$0.age = $0.age, _2: select * from $3 <- students, $4 <- professors where s$0.age = $3.age) from s$0 <- students, $2   <- professors""",
      TestWorlds.professors_students)
  }
