...
/** Algorithm that converts a calculus expression (in canonical form) into the logical algebra.
  ...
  */
trait Unnester2 extends Canonizer {
  ...
  lazy val unnester = manytd(unnest)
  ...
  def recurse(t: Term): Term =
    t match {
      ...
      /** Rule C12.
        */
      case CalculusTerm(CanComp(m, Nil, p, f@NestedComp(c)), u, Some(w), child) =>
      ...
      case CalculusTerm(CanComp(m, Gen(pat_v @ PatternIdn(IdnDef(v)), path) :: r, p, e), None, Some(w), AlgebraTerm(child)) =>
        ...
        val pat_w_v = PatternProd(Seq(w, pat_v))
        val (pred_v, pred_not_v) = p.partition(variables(_) == Set(v))
        recurse(CalculusTerm(CanComp(m, r, pred_not_v, e), None, Some(pat_w_v), AlgebraTerm(Unnest(Gen(w, child), Gen(pat_w_v, path), foldPreds(pred_v)))))
      ...
      case CalculusTerm(CanComp(m, Nil, p, e), Some(u), Some(w), AlgebraTerm(child)) =>
        ...
        AlgebraTerm(Nest(m, Gen(w, child), createRecord(u, w), e))
      ...
      case CalculusTerm(CanComp(m, Gen(pat_v @ PatternIdn(IdnDef(v)), path) :: r, p, e), Some(u), Some(w), AlgebraTerm(child)) =>
        ...
        val pat_w_v = PatternProd(Seq(w, pat_v))
        val (pred_v, pred_not_v) = p.partition(variables(_) == Set(v))
        recurse(CalculusTerm(CanComp(m, r, pred_not_v, e), Some(u), Some(pat_w_v), AlgebraTerm(OuterUnnest(Gen(w, child), Gen(pat_w_v, path), foldPreds(pred_v)))))
    }
  ...
  /** Returns true if the comprehension `c` does not depend on `s` generators.
    */
  def areIndependent(c: Comp, s: Seq[Gen]) = {
