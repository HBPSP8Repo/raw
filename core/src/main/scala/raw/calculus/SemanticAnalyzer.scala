...
//  lazy val partitionEnv: Chain[Environment] =
...
//  }
      ...
      // TODO: partition is generated by a group by - it only exists after a group by.
    ...
    case VariableEntity(idn, t) => {
      ...
      dcl match {
        ...
        case Some(g: Gen)       =>
          // Add all pattern identifier types to the map before processing the rhs
          // This call is repeated multiple times in case of a PatternProd on the lhs of the Bind. This is harmless.
          patternIdnTypes(g.p).foreach{ case pt => typesVarMap.union(pt, pt)}
...
//      logger.debug(s"Unifying t1 ${TypesPrettyPrinter(t1)} and t2 ${TypesPrettyPrinter(t2)}")
      ...
      case PartitionHasType(s) =>
        ...
        def fromType(from: Seq[Iterator]): Type =
          if (from.length == 1) {
            ...
            find(t) match {
              case CollectionType(_, innerType) => CollectionType(BagMonoid(), innerType)
              case _ => ??? // Add error to errors list
            }
          }
          else if (from.length > 1)
            CollectionType(BagMonoid(), RecordType(from.map { case Iterator(Some(PatternIdn(IdnDef(idn))), e) => AttrType(idn, expType(e)) }, None))
          else
  ...
  def constraint(n: Exp): Seq[Constraint] = {
    ...
    n match {
      ...
      case s @ Select(from, _, _, _, _, _, _) =>
        Seq(
          PartitionHasType(s))
...
//        if e1 or e2 are options, then n is an option, but the type of n is the inner type of e1/e2
      ...
      // Rule 14
      case Comp(m: CollectionMonoid, qs, e1) =>
        qs.collect { case Gen(_, e) => e }.map(e => ExpMonoidSubsetOf(e, m)) ++
